...
saveRDS(df_final, "df_ready_for_ML.rds")
```
...
```{r}
set.seed(5436)
df <- read_rds("df_ready_for_ML.rds") %>% select(-booking_change)
...
With Logistic Regression model, we improved the accuracy by 6% compared to the baseline model. Considering that it is not a very computationally intensive training, this could be a great model for our problem. We will check the model's performance on test data and see whether the model actually gives better results than the baseline model.
...
The Logistic Regression performs very well on the test data set, with 98% of accuracy and increased specificity to 86% while sensitivity trade-off is only around 0.4%. We can modify the threshold to improve this.

```{r}
test_results$logit <- predict(logit, classTest, type = "prob") %>% pull(Yes)
test_results$logit <- ifelse(test_results$logit > 0.4, "Yes", "No") %>% factor(c("Yes", "No"), levels = c("Yes", "No"))
confusionMatrix(test_results$logit, test_results$test)
```

It seems that at threshold of 0.4 to be classified as positive cancellation prediction, we get the best model with accuracy of 0.98 and increasing specificity slightly by 1% while maintaining sensitivity.

### Bayes Classifiers

```{r}
# Model train
lda <- lda(classTrain[,-14], grouping = classTrain[,14])
lda
```
...
Similarly to the performance on train data, the model did not improve much the accuracy of predictions compared to the baseline model. Specificity improved slightly and there is little trade-off on sensitivity but overall it is still not a good model in practice.
...
```{r}
gbTune <- expand.grid(nrounds = c(500,1000), max_depth = c(5,6,7), eta = c(0.01, 0.1, 1),
...
```{r}
# nn <- train(cancelled ~. , data = classTrain,
#             method = "nnet",
#             trControl = ctrl,
#             tuneGrid = expand.grid(size = c(5),
#                                    decay = c(0.1)))
# saveRDS(nn, "models/classification/nn.rds")
# nn
```
...
```{r}
# inverse log
# unscaled <- classTrain$nightly_rate_usd * 0.109 + 0.448
...
On test data set, the model performs better, with RMSE ofo 0.76 and R^2^ of 0.38. This is slightly worse than the multi-linear regression model and takes more time to train so the linear model is still better in practice.
...
After tuning, our Gradient Boosting Machine model slightly improved with RMSE around 0.60 and R^2^ around 0.63. This is only a slight improvement considering the computation time was a lot longer. It seems that better tunning might be necessary to see a significant improvement in the model. It seems that to get better performance, we would need to try with higher nrounds, max_depth = 5, lower eta than 0.1, gamma around 1, colsample_bytree betwene 0.6 and 1, min_child_weight = 1, and subsample lower than 1,
...
```{r}
nn <- train(nightly_rate_usd ~ hotel_stars + amenities + agency_cancellation + hotel_cancellation + leadtime + hotel_latitude + hotel_longitude + free_breakfast + bed_type + bed_number + checkin_month, # Removed some features with many levels
            data = regTrain,
            method = "neuralnet",
            trControl = ctrl,
            preProcess = c("nzv"),
...
saveRDS(nn, "models/regression/nn.rds")
```

```{r}
test_results$nn <- predict(nn, regTest)

postResample(pred = test_results$nn,  obs = test_results$test)
```
