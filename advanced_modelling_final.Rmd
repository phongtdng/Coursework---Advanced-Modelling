...
df <- clean_data %>% 
  ...
  mutate(across(c(free_wifi, free_breakfast, free_parking, concierge, cancelled, checkin_month, booking_change, hotel_country), as.factor)) %>% 
...
transformation <- transformation %>%
  mutate(log_nightly_rate = log(nightly_rate_usd),
         log_total_cost = log(total_cost_usd),
         log_lead_time = log(leadtime),
         log_nights = log(nights),
         sq_client_cancellation = client_cancellation^2,
         sq_agency_cancellation = agency_cancellation^2,
         sq_hotel_stars = hotel_stars^2
  )
```
